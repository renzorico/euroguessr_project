'''
NOTE: Make sure the commands are run in the `BCNGUESSR/ML/` directory
...
class Geoguessr:
    ...
    def __init__(self, model=None, loss=-1, useRestnet = True, 
                 inputShape=(3, 300, 600, 3), gridCount=56,
                 modelOptimizer=None):
        '''
        ...
        if modelOptimizer is None:
            # Use the default Adam optimizer if modelOptimizer is not provided
    ...
    def GridPrediction(self, imgFiles, ployGrid):
        '''
        Predicts the grids where the model has more confidence that the new triplet image is located.
        ImgFile: String that contains the test location image triplet folder name.
        The string has to look like: <gridNo>+<lat,long>, eg: 60+48.4271513,-110.5611851
        dataDir: Directory that stores combined image files, eg: "/dataCombinedSamples/"
        threshold: Confidence threshold to filter the predicted grids. Grids with probabilities greater than the threshold are considered confident predictions.
        '''
        
        # Load the images and convert them to arrays
        xx = [img_to_array(Image.open(file).resize((600, 300))) for file in imgFiles]
        # Predict single image triplet
        yp = self.model.predict(np.array([xx]))[0]
        # Find the grid with the highest confidence (maximum probability)
        confident_grid = np.argmax(yp)
        # Plot the BcnPolyGrid with the confident grids
        for k,i in ployGrid.items():
            plt.plot(i[:,1],i[:,0],color='black', alpha=0.8)
            if k == confident_grid:
                poly = Polygon(i)
                minx, miny, maxx, maxy = poly.bounds
                pnt = Point(random.uniform(minx, maxx), random.uniform(miny, maxy))
                location = str(round(pnt.x,5))+','+str(round(pnt.y,5))
                print("The predicted location is: ", location)
                plt.fill(i[:, 1], i[:, 0], color='red', alpha= 0.75)
                plt.scatter(pnt.y, pnt.x, color='blue', marker="x", s=100)
                
        plt.xlabel('Longitude')
        plt.ylabel('Latitude')
        plt.title('BCN Predicted Location')
        
        pred_plot = plt.savefig('prediction_plot.png')
    ...
    def predictSingle(self, imgFile, dataDir, ployGrid):
        ...
        ImgFiles: String that contains test location image triplet folder name. String has to look like:
        # <gridNo>+<lat,long>
        # 60+48.4271513,-110.5611851
        dataDir: Directory that stores combined image files eg: "/dataCombinedSamples/"
        polyGrid: List of polygons that contain make up the BCN split into grids.
                  It can be loaded from eg: "infoExtraction/bcnPolyGrid.pkl"
        '''
        ...
        plt.plot([start[1],end[1]], [start[0],end[0]], color='black', 
                 label="Distance: {} miles".format(round(dist,2)))
        for k,i in ployGrid.items():
            if k==np.argmax(yy[0]):
